  ...
  return (
    ...
    >
      <div className="relative aspect-video w-full bg-bg-page border-b border-divider/50 overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute inset-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
          <div className="flex justify-between items-start w-full">
            ...
            >
              <div className={`w-7 h-7 rounded-[6px] flex items-center justify-center transition-all ${isSelected ? 'bg-brand text-white' : 'bg-white/80 text-black/40 hover:bg-white hover:text-black/60'}`}>
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${isFavorite ? 'text-warning bg-white shadow-sm' : 'text-black/40 hover:text-warning hover:bg-white'}`}>
      ...
      <div className="px-4 py-4 flex items-center justify-between gap-3 min-h-[60px] bg-bg-card">
        {/* T5 Body Strong (14px/22px, 500 weight) */}
        <VFText variant="t5-strong" as="h3" className="flex-1 m-0" truncate>
          {workflow.name}
        </VFText>
