  ...
  const LegendItem = ({ color, label, pct }: { color: string, label: string, pct: number }) => (
    <div className="flex items-center justify-between text-sm py-1">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-normal text-text-secondary leading-[18px]">{label}</span>
      </div>
    ...
    <VFCard className="h-full relative" noPadding>
      <div className="p-6 pb-2">
        ...
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="text-3xl font-semibold text-text-primary tracking-tight leading-[40px] tabular-nums">
            {data.total}
          </div>
